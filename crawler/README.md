...
- **定时调度**：支持按小时间隔定时执行
- **日前联络线计划批量查询**：支持通过文件输入「日期+联络线名称」批量执行指定查询导出（`--batch-file`）
...
| `--batch-file` | **日前联络线计划专用**：从文件读取「日期+筛选条件」批量执行 | - | `--batch-file batch.txt` |

---

### 日前联络线计划：按文件批量查询

针对任务 **日前联络线计划**，支持通过 `--batch-file` 指定一个文本文件，按「日期 + 联络线名称」执行**指定查询并导出**，支持批量处理，且不影响其他任务的执行逻辑。

**批量文件格式（UTF-8 编码）：**

- 每行一条查询，两种写法：
  - **两列**：`日期,联络线名称` → 单日、单条联络线
  - **三列**：`开始日期,结束日期,联络线名称` → 日期范围内该联络线（按日逐日执行）
- 空行、以 `#` 开头的行会被忽略
- 日期格式必须为 `YYYY-MM-DD`

**示例文件 `tieline_batch.txt`：**
...
2026-01-02,山西-山东
# 日期范围 + 联络线（会按日展开为多天）
2026-01-05,2026-01-10,山西-河北
```

**运行方式：**

```bash
# 仅执行「日前联络线计划」，按批量文件中的日期+联络线逐条查询导出
python main.py --task 日前联络线计划 --batch-file tieline_batch.txt
```

- 使用 `--batch-file` 时，**必须**同时指定 `--task 日前联络线计划`（可含其他任务，如 `--task "日前联络线计划,断面约束"`；仅 日前联络线计划 会使用批量文件，其他任务仍按 `--start` / `--end` 执行）。
- `--start` / `--end` 在批量模式下仅作为默认日期（例如未指定时），**日前联络线计划**的实际日期以批量文件为准。
- 不指定 `--batch-file` 时，日前联络线计划仍按原逻辑：在 `--start`～`--end` 范围内按日遍历，并对页面全部「联络线名称」下拉选项逐一查询导出。
...
- **页面自动刷新恢复**：通过 iframe ID 比对检测页面跳转，自动重新导航
