...
| 字段 | 说明 | 默认值 |
|------|------|--------|
| `max_rows_to_scan` | 读取 Excel 时最大行数 | `100` |
| `scan_rows_for_content` | 扫描内容时最大行数 | `60` |
...
输出包括各 header 行下的列名和前 5 行数据，以及无 header 模式下的原始内容。
...
2025-01-05,山西送河北(省间现货)
2025-01-05,总加
```

**`validation_errors.txt`** — 错误详细报告，分三节：
1. 日期校验错误（文件名日期与表格内容不符）
2. 通道校验错误（文件名通道与表格内容不符）
3. 综合统计（日期错误数 / 通道错误数 / 两者均错误数）
